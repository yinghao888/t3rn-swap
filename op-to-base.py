...
def initialize_web3():
    ...
    return Web3(Web3.HTTPProvider(op_rpcs[0]))
...
def bridge_op_to_base(w3_op, account, amount_eth=1):
    """从 Optimism 跨到 Base"""
    try:
...
def main():
    ...
    logging.info(f"开始为 {len(accounts)} 个账户执行 OP->BASE 单向跨链，每次 5 ETH")
    ...
    while True:
        ...
        for account in accounts:
            try:
                # OP -> BASE
                if bridge_op_to_base(w3_op, account, 5):
                    # 等待 1-2 秒
