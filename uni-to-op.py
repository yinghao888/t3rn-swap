...
def create_data_for_uni_to_op(address):
    """根据用户地址创建交易数据"""
    # 去除地址前缀0x
...
def bridge_uni_to_op(w3_uni, account, amount_eth=1):
    """从 Unichain 跨到 Optimism"""
    try:
        amount_wei = w3_uni.to_wei(amount_eth, 'ether')
        nonce = w3_uni.eth.get_transaction_count(account['address'])
        
        # 创建带有用户地址的数据
        data = create_data_for_uni_to_op(account['address'])
...
def main():
    ...
    logging.info(f"开始为 {len(accounts)} 个账户执行 UNI->OP 单向跨链，每次 5 ETH")
    ...
    while True:
        ...
        for account in accounts:
            try:
                # UNI -> OP
                if bridge_uni_to_op(w3_uni, account, 5):
                    # 等待 1-2 秒
